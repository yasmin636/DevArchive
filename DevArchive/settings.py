"""
Django settings for the SIGAEUD project (package Python « DevArchive »).
...
def _load_dotenv_file() -> None:
    """Charge un .env local simple (KEY=VALUE) sans dépendance externe."""
...
# Provider de paiement de l'abonnement étudiant.
# Valeurs prévues : mastercard_demo (actuel), manual (fallback historique).
PAYMENT_PROVIDER = os.environ.get("PAYMENT_PROVIDER", "mastercard_demo").strip().lower()
MASTERCARD_MERCHANT_ID = os.environ.get("MASTERCARD_MERCHANT_ID", "").strip()
MASTERCARD_API_PASSWORD = os.environ.get("MASTERCARD_API_PASSWORD", "").strip()
MASTERCARD_GATEWAY_URL = os.environ.get("MASTERCARD_GATEWAY_URL", "https://eu-gateway.mastercard.com").strip()
MASTERCARD_CURRENCY = os.environ.get("MASTERCARD_CURRENCY", "USD").strip().upper()
MASTERCARD_API_VERSION = os.environ.get("MASTERCARD_API_VERSION", "100").strip()
...
# Cookies marqués Secure (recommandé en HTTPS derrière ngrok ou un reverse-proxy).
# PowerShell : $env:DJANGO_SECURE_COOKIES="true"
if os.environ.get("DJANGO_SECURE_COOKIES", "").strip().lower() in {"1", "true", "yes", "on"}:
